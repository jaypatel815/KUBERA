...
- [ ] T070 — Voice v0: push-to-talk desktop loop (owner is voice-first, D015): `scripts\talk.py` — hold a key, record mic, STT (faster-whisper local or OpenAI STT via existing key, configurable), POST /api/chat with voice=true + conversation threading, speak the reply via TTS (pyttsx3/Windows SAPI or edge-tts). Acceptance: full spoken round-trip on the owner's machine; confirmation NEVER inferred from speech — a distinct deliberate gesture (e.g. typed RESET-style phrase or key chord) sets confirm=true. Builds on the voice flag shipped 2026-08-12.
