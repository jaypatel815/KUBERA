...
http://127.0.0.1:8000/api/pattern-warnings/SPY              pre-trade pattern warnings: 0DTE expectancy, revenge
                                                            sizing drift, post-loss tilt tempo, symbol track record
...
**Earnings intelligence rides two free lines in `.env`** (`EDGAR_CONTACT=` — the SEC
requires a contact and blocks anonymous clients — and `FINNHUB_API_KEY=`, free from
finnhub.io): ask *"should I hold through NVDA earnings?"* and KUBERA answers with that
symbol's OWN base rates — how past earnings actually resolved, timed by real SEC filing
clocks, split by real beats and misses (Finnhub's actual-vs-estimate history, matched
to report dates only when unambiguous), never a prediction. Ask *"preview NVDA's
earnings"* for the full setup: report date, base rates, the realized 1-day range, your
exposure — no price targets, by design. Ask *"what did Apple's earnings release say?"*
and it reads the press release (exhibit 99.1) straight from EDGAR. Personal-finance
extras from the same recorded fills: *"any tax losses worth harvesting?"* (wash-sale
windows from your own buys — NOT tax advice, and it says so) and *"what's my thesis on
SPY?"* (your watchlist note + journaled decisions + the current invalidation level +
upcoming catalysts — composed from your own record, never invented).

**Install KUBERA as a Claude plugin:** the repo doubles as a plugin marketplace
(`.claude-plugin/`). In Claude Code: `claude plugin marketplace add <repo-path>` then
`claude plugin install kubera@kubera` — you get `/kubera:resume` (session resume) and
`/kubera:connect` (walks through wiring the 44-tool MCP surface; the machine-local
config is generated by `scripts/install_mcp_config.py`, never shipped).
...
python scripts\pattern_check.py SPY --dte 0     # Pre-trade pattern check against historical records
```
...
- `kokoro` — **(RECOMMENDED, D024)** local near-human neural voice (free, offline, 50+ voices). Reply text (holdings, dollar P&L) stays on this machine: `pip install kokoro-onnx`, place `kokoro-v1.0.onnx` + `voices-v1.0.bin` in `models/kokoro/` (or set `KUBERA_KOKORO_DIR`), `set KUBERA_TTS=kokoro`, `set KUBERA_VOICE=af_heart`.
