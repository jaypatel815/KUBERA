...
def household(session=Depends(get_db_session)) -> dict:
    ...
    so. T155 moved the composition to household_service so the chat tool and
    this endpoint can never disagree; the 503 mapping stays here."""
    from api.household_service import ALEMBIC_HINT, compose_household

    try:
        return compose_household(session)
    except OperationalError:
        raise HTTPException(status_code=503, detail=ALEMBIC_HINT)
